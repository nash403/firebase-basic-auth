<template>
  <div class="md:flex mx-4 md:mx-64 md:flex-wrap rounded-lg p-4 border border-gray-300 relative">
    <div v-if="showLoader.value" class="absolute inset-0 flex z-10">
    ...
    <div class="md:flex-shrink-0 md:mr-6 md:mb-1">
      <img class="rounded-lg md:w-56 md:min-h-full bg-gray-400" :src="user && user.photoURL ? user.photoURL : '@/assets/undraw_male_avatar_323b.png'" alt="Signed in user profile picture" />
    </div>
    <div class="mt-4 md:mt-0">
      <div class="uppercase tracking-wide text-sm text-indigo-600 font-bold">
        Firebase Google Authentication ({{ showLoader.value ? 'loading...' : 'loaded' }})
      </div>
  ...
  export default defineComponent({
    ...
    setup () {
      ...
      return {
        ...
        user,
        error,
        showLoader: computed(() => !ready || signingIn),
      }
    },
  })
</script>
